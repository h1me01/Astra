...
#define PARAM(name, value, min, max, step) constexpr int name = value
    ...
    // search parameters
    PARAM(lmr_base, 97, 80, 120, 8);
    PARAM(lmr_div, 181, 150, 200, 8);

    PARAM(asp_depth, 6, 5, 9, 1);
    PARAM(asp_window, 6, 5, 30, 5);

    PARAM(rzr_depth_mult, 193, 150, 250, 15);
    PARAM(rfp_depth_mult, 77, 70, 110, 6);

    PARAM(nmp_min, 4, 3, 6, 1);
    PARAM(nmp_depth_div, 6, 3, 15, 1);
    PARAM(nmp_div, 219, 190, 235, 8);

    PARAM(probcut_margin, 152, 130, 180, 15);

    PARAM(see_cap_margin, 99, 80, 110, 6);
    PARAM(see_quiet_margin, 48, 30, 60, 6);

    PARAM(fp_base, 168, 120, 180, 8);
    PARAM(fp_mult, 94, 80, 150, 8);

    PARAM(ext_margin, 124, 45, 150, 12);

    PARAM(hp_margin, 4920, 3500, 5000, 400);
    PARAM(hp_div, 7940, 7000, 8500, 400);
    PARAM(hbonus_margin, 78, 65, 80, 5);

    PARAM(qfp_margin, 86, 60, 150, 15);
