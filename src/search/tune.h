...
#define PARAM(name, value, min, max, step) constexpr int name = value
    ...
    PARAM(lmr_div, 189, 150, 200, 8);

    PARAM(asp_depth, 6, 5, 9, 1);
    PARAM(asp_window, 8, 5, 30, 5);

    PARAM(rfp_depth, 8, 7, 9, 1);
    PARAM(rfp_depth_mult, 90, 70, 110, 6);

    PARAM(rzr_depth_mult, 207, 150, 250, 15);

    PARAM(nmp_depth_mult, 32, 27, 34, 1);
    PARAM(nmp_base, 189, 160, 200, 8);
    ...
    PARAM(see_cap_margin, 88, 80, 110, 6);
    PARAM(see_quiet_margin, 41, 30, 60, 6);

    PARAM(hp_depth, 7, 5, 8, 1);

    PARAM(fp_depth, 9, 7, 11, 1);
    PARAM(fp_base, 158, 120, 180, 8);
    PARAM(fp_mult, 104, 80, 150, 8);

    PARAM(ext_margin, 117, 65, 150, 12);

    PARAM(hp_margin, 4939, 3500, 5000, 400);
    PARAM(hp_div, 7969, 7000, 8500, 400);
    ...
    PARAM(max_history_bonus, 1541, 1100, 1750, 50);
