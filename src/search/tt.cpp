...
void freeAlign(void *ptr) { _mm_free(ptr); }
...
TTEntry *TTable::lookup(U64 hash, bool *hit) const {
    ...
    for (int i = 0; i < BUCKET_SIZE; i++)
        if (entries[i].getHash() == hash16 || !entries[i].getDepth()) {
            uint8_t age_pv_bound =
                (uint8_t)(tt.getAge() | (entries[i].getAgePvBound() & (AGE_STEP - 1)));
    ...
    for (int i = 1; i < BUCKET_SIZE; i++) {
        int worst_value = worst_entry->getDepth() -
                          ((AGE_CYCLE + tt.getAge() - worst_entry->getAgePvBound()) & AGE_MASK) / 2;
        int entry_value = entries[i].getDepth() -
                          ((AGE_CYCLE + tt.getAge() - entries[i].getAgePvBound()) & AGE_MASK) / 2;
