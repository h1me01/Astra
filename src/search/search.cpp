...
void Search::start() {
    ...
    for(int i = 0; i < MAX_PLY + 6; i++) {
        ...
        stack_arr[i].conth = &history.conth[0][0][NO_PIECE][NO_SQUARE];
    }
...
Score Search::negamax(int depth, Score alpha, Score beta, Stack *stack, bool cut_node) {
    ...
    if(cut_node                                                          //
       ...
       && stack->static_eval + nmp_depth_mult * depth - nmp_base >= beta //
        ...
        stack->conth = &history.conth[0][0][NO_PIECE][NO_SQUARE];
...
void Search::make_move(const Move &move, Stack *stack) {
    ...
    stack->conth = &history.conth[board.in_check()][move.is_cap()][stack->moved_piece][move.to()];
