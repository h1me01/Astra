...
U64 slidingAttacks(Square sq, const U64 occ, const U64 mask) {
    ...
    return ((mask_occ - SQUARE_BB[sq] * 2) ^
            (reverse(reverse(mask_occ) - reverse(SQUARE_BB[sq]) * 2))) &
           mask;
}
...
#endif
            ROOK_ATTACKS[s][idx] = slidingAttacks(s, blockers, MASK_FILE[fileOf(s)]) |
...
#endif
            BISHOP_ATTACKS[s][idx] = slidingAttacks(s, blockers, MASK_DIAGONAL[diagOf(s)]) |
