...
#endif
            ROOK_ATTACKS[s][idx] = sliding_attacks(s, blockers, MASK_FILE[sq_file(s)]) |
                                   sliding_attacks(s, blockers, MASK_RANK[sq_rank(s)]);
...
#endif
            BISHOP_ATTACKS[s][idx] = sliding_attacks(s, blockers, MASK_DIAGONAL[sq_diag(s)]) |
                                     sliding_attacks(s, blockers, MASK_ANTI_DIAGONAL[sq_antidiag(s)]);
...
void init_lookup_tables() {
    ...
    for(Square s = a1; s <= h8; ++s) {
        ...
        PSEUDO_LEGAL_ATTACKS[ROOK][s] = get_rook_attacks(s, 0);
        PSEUDO_LEGAL_ATTACKS[BISHOP][s] = get_bishop_attacks(s, 0);
    ...
    for(Square s1 = a1; s1 <= h8; ++s1) {
        for(Square s2 = a1; s2 <= h8; ++s2) {
            ...
            if(sq_file(s1) == sq_file(s2) || sq_rank(s1) == sq_rank(s2)) {
                U64 b1 = get_rook_attacks(s1, s);
                U64 b2 = get_rook_attacks(s2, s);
                SQUARES_BETWEEN[s1][s2] = b1 & b2;

                b1 = get_rook_attacks(s1, 0);
                b2 = get_rook_attacks(s2, 0);
                LINE[s1][s2] = (b1 & b2) | SQUARE_BB[s1] | SQUARE_BB[s2];
            } else if(sq_diag(s1) == sq_diag(s2) || sq_antidiag(s1) == sq_antidiag(s2)) {
                U64 b1 = get_bishop_attacks(s1, s);
                U64 b2 = get_bishop_attacks(s2, s);
                SQUARES_BETWEEN[s1][s2] = b1 & b2;

                b1 = get_bishop_attacks(s1, 0);
                b2 = get_bishop_attacks(s2, 0);
                LINE[s1][s2] = (b1 & b2) | SQUARE_BB[s1] | SQUARE_BB[s2];
            }
        }
    }
