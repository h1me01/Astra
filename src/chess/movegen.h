...
inline Move *genLegals(const Board &board, Move *ml) {
    ...
    while (curr != ml)
        if (((pinned & SQUARE_BB[curr->from()]) || curr->type() == EN_PASSANT) &&
            !board.isLegal(*curr))
            *curr = *(--ml);
        else
            ++curr;
...
public:
    ...
    constexpr Type &operator[](int i) { return list[i]; }

    template <GenType gt>
    void gen(const Board &board) {
        static_assert(std::is_same<Type, Move>::value,
                      "MoveList type must be Move to generate moves.");
