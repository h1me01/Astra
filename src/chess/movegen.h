...
namespace Chess {
    ...
    constexpr U64 ooMask(Color c) {
        return c == WHITE ? WHITE_OO_MASK : BLACK_OO_MASK;
    }

    constexpr U64 oooMask(Color c) {
        return c == WHITE ? WHITE_OOO_MASK : BLACK_OOO_MASK;
    }

    constexpr U64 ooBlockersMask(Color c) {
        return c == WHITE ? 0x60 : 0x6000000000000000;
    }

    constexpr U64 oooBlockersMask(Color c) {
        return c == WHITE ? 0xe : 0xE00000000000000;
    }

    constexpr U64 ignoreOOODanger(Color c) {
    ...
    public:
        explicit MoveList(Board &board, MoveType mt = ALL_MOVES) {
            ...
            if (mt == CAPTURE_MOVES) {
                last = is_w ? genLegalMoves<WHITE, CAPTURE_MOVES>(board, list) :
                              genLegalMoves<BLACK, CAPTURE_MOVES>(board, list);
            } else {
