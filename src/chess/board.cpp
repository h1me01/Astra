...
Board::Board(const std::string &fen, bool update_nnue) { setFen(fen, update_nnue); }
...
bool Board::isLegal(const Move &m) const {
    ...
    if (m.type() == EN_PASSANT) {
        ...
        U64 attackers =
            getBishopAttacks(ksq, occ) & (getPieceBB(~stm, BISHOP) | getPieceBB(~stm, QUEEN));
...
bool Board::isPseudoLegal(const Move &m) const {
    ...
    if (mt == EN_PASSANT) {
        if (pt != PAWN ||        // make sure we move a pawn
            ...
            pieceAt(Square(info.ep_sq ^ 8)) !=
                makePiece(~stm, PAWN)) // pawn on ep square must be a enemy pawn
        {
            return false;
        }
    }
    ...
    if (isProm(m)) {
        ...
        U64 targets =
            info.checkers ? SQUARES_BETWEEN[ksq][lsb(info.checkers)] | info.checkers : -1ULL;
...
void Board::makeMove(const Move &m, bool update_nnue) {
    ...
    if (mt == CASTLING) {
        ...
        info.non_pawn_hash[stm] ^=
            Zobrist::getPsq(rook, rook_from) ^ Zobrist::getPsq(rook, rook_to);
...
void Board::initThreats() {
    ...
    Color them = ~stm;
    U64 occ = occupancy() ^
              SQUARE_BB[kingSq(stm)]; // king must be excluded so we don't block the slider attacks
...
void Board::initCheckersAndPinned() {
    ...
    U64 candidates = (getRookAttacks(ksq, their_occ) & orthSliders(them)) |
                     (getBishopAttacks(ksq, their_occ) & diagSliders(them));
