...
namespace NNUE
{
    ...
    {
        memcpy(acc.getData(WHITE), nnue.ft_biases, sizeof(int16_t) * HIDDEN_SIZE);
        memcpy(acc.getData(BLACK), nnue.ft_biases, sizeof(int16_t) * HIDDEN_SIZE);
    }

    int32_t NNUE::forward(Board &board) const
    {
        board.updateAccumulator();
